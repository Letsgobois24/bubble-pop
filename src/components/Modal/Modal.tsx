...
type PropsType = {
  ...
  onClose?: () => void;
  title: string;
  size: "max-w-xl" | "max-w-md";
};

const Modal = ({ children, onClose, title, size }: PropsType) => {
  const ref = useRef<HTMLDivElement>(null);

  return (
    <div className="flex justify-center items-center fixed top-0 right-0 left-0 bottom-0 z-50 bg-black/50">
      <div className={`${size} py-4 w-full max-h-full`}>
        <div
          ref={ref}
          className="bg-white rounded-lg shadow-sm max-h-[90vh] overflow-y-auto"
        >
